...
// Loads the actual photo used as the meme background: either a
// user-supplied image URL (uploaded photo, Catbox link, any public CDN
// link) or, if none was given, the bundled demo photo.
async function loadBaseImageBuffer({ origin, imageUrl }) {
  if (!imageUrl) {
...
export async function renderStaticMeme({ width, height, topText, bottomText, color, stroke, format, origin, imageUrl }) {
  const baseBuffer = await loadBaseImageBuffer({ origin, imageUrl });
...
export async function renderGifMeme({ width, height, topText, bottomText, color, stroke, origin, imageUrl }) {
  const rawBase = await loadBaseImageBuffer({ origin, imageUrl });
...
export function parseCommonParams(searchParams) {
  ...
  return { topText, bottomText, width, height, format, color, stroke, imageUrl };
}
